export default function estadoPedidosAlfruitTestManager() {
    return {
        ...
        _endpoint: 'estado-pedidos-alfruit-test',
        ...
        async loadEstadoActual() {
            try {
                ...
                console.error("Error cargando estado actual Alfruit Test:", err);
            } finally {
                this.loaded = true;
            }
        },
        ...
        async loadHistorial() {
            try {
                const res = await fetch(`http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-alfruit-test/historial${this._centroQuery()}`);
                ...
                console.error("Error cargando historial Alfruit Test:", err);
            }
        },
        ...
        async openPedidoDetail(item) {
            ...
            try {
                ...
                const urlHeader = usarNumPedido
                    ? `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-alfruit-test/pedido-by-num/${numPedido}`
                    : `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-alfruit-test/pedido/${idpedido}`;
                ...
                if (this.pedidoDetail?.PED_idpedido) {
                    const urlLineas = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-alfruit-test/pedido-lineas/${this.pedidoDetail.PED_idpedido}`;
                ...
                if (cliente) {
                    ...
                    const urlProd = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-alfruit-test/pedido-prod?${params.toString()}`;
                ...
                console.error("Error cargando detalle pedido Alfruit Test:", err);
            }
        },
