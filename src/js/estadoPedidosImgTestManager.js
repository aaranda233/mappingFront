export default function estadoPedidosImgTestManager() {
    return {
        current: null,
        historial: [],
        loaded: false,
        ...
        _endpoint: 'estado-pedidos-img-test',
        ...
        async openPedidoDetail(item) {
            ...
            try {
                ...
                const urlHeader = usarNumPedido
                    ? `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-img-test/pedido-by-num/${numPedido}?cliente=${encodeURIComponent(item.cliente ?? '')}`
                    : `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-img-test/pedido/${idpedido}`;
                ...
                if (this.pedidoDetail?.PED_idpedido) {
                    const urlLineas = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-img-test/pedido-lineas/${this.pedidoDetail.PED_idpedido}`;
                ...
                if (cliente) {
                    ...
                    const urlProd = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-img-test/pedido-prod?${params.toString()}`;
                ...
                console.error("Error cargando detalle pedido Img Test:", err);
            }
        },
