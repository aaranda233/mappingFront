export default function estadoPedidosEurogroupTestManager() {
    return {
        ...
        async loadHistorial() {
            try {
                const res = await fetch(`http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-eurogroup-test/historial${this._centroQuery()}`);
        ...
        async openPedidoDetail(item) {
            ...
            try {
                ...
                const urlHeader = usarNumPedido
                    ? `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-eurogroup-test/pedido-by-num/${numPedido}`
