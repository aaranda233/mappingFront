export default function estadoPedidosLehmanTestManager() {
    return {
        current: null,
        historial: [],
        loaded: false,
        ...
        _endpoint: 'estado-pedidos-lehman-test',
        ...
        async openPedidoDetail(item) {
            ...
            try {
                ...
                const urlHeader = usarNumPedido
                    ? `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-lehman-test/pedido-by-num/${numPedido}?cliente=${encodeURIComponent(item.cliente ?? '')}`
                    : `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-lehman-test/pedido/${idpedido}`;
                ...
                if (this.pedidoDetail?.PED_idpedido) {
                    const urlLineas = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-lehman-test/pedido-lineas/${this.pedidoDetail.PED_idpedido}`;
                ...
                if (cliente) {
                    ...
                    const urlProd = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-lehman-test/pedido-prod?${params.toString()}`;
                ...
                console.error("Error cargando detalle pedido Lehman Test:", err);
            }
        },
