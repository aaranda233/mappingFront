export default function estadoPedidosAnecoopTestManager() {
    return {
        current: null,
        historial: [],
        loading: false,
        ...
        async loadEstadoActual() {
            try {
                const res = await fetch(`http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-anecoop-test/actual?centro=${this._centro}`);
        ...
        async loadHistorial() {
            try {
                const res = await fetch(`http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-anecoop-test/historial?centro=${this._centro}`);
        ...
        async openPedidoDetail(item) {
            ...
            try {
                ...
                const urlHeader = usarNumPedido
                    ? `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-anecoop-test/pedido-by-num/${numPedido}`
                    : `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-anecoop-test/pedido/${idpedido}`;
                ...
                if (this.pedidoDetail?.PED_idpedido) {
                    const urlLineas = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-anecoop-test/pedido-lineas/${this.pedidoDetail.PED_idpedido}`;
                ...
                if (cliente) {
                    ...
                    const urlProd = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-anecoop-test/pedido-prod?${params.toString()}`;
                ...
                console.error("Error cargando detalle pedido Anecoop Test:", err);
            }
        },
