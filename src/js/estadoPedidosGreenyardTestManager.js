export default function estadoPedidosGreenyardTestManager() {
    return {
        ...
        _endpoint: 'estado-pedidos-greenyard-test',
        ...
        async openPedidoDetail(item) {
            ...
            try {
                ...
                const urlHeader = usarNumPedido
                    ? `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-greenyard-test/pedido-by-num/${numPedido}?cliente=${encodeURIComponent(item.cliente ?? '')}`
                    : `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-greenyard-test/pedido/${idpedido}`;
                ...
                console.error("Error cargando detalle pedido Greenyard Test:", err);
            }
        },
