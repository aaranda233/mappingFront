...
export default function estadoPedidosDaifresshTestManager() {
    return {
        ...traspasoProduccion(),
        current: null,
        historial: [],
        ...
        _endpoint: 'estado-pedidos-daifressh-test',
        ...
        async openPedidoDetail(item) {
            ...
            try {
                ...
                const urlHeader = usarNumPedido
                    ? `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-daifressh-test/pedido-by-num/${numPedido}?cliente=${encodeURIComponent(item.cliente ?? '')}`
                    : `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-daifressh-test/pedido/${idpedido}`;
                ...
                if (this.pedidoDetail?.PED_idpedido) {
                    const urlLineas = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-daifressh-test/pedido-lineas/${this.pedidoDetail.PED_idpedido}`;
                ...
                if (cliente) {
                    ...
                    const urlProd = `http://${window.env.IP_BACKEND}/api/mapping/estado-pedidos-daifressh-test/pedido-prod?${params.toString()}`;
                ...
                console.error("Error cargando detalle pedido Daifressh Test:", err);
            }
        },
